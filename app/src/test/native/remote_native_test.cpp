...
public:
    int64_t value;
    TestClock() : value(0) {}
    virtual int64_t nowMs() const { return value; }
};

class TestSink : public CommandSink {
public:
    std::vector<NativeCommand> commands;
    virtual bool dispatch(const NativeCommand &command) { commands.push_back(command); return true; }
};
...
int main() {
    ...
    assert(parseHttpRequest("GET /../secret HTTP/1.0\r\n\r\n", config, &request, &consumed) == PARSE_BAD_REQUEST);

    TestClock clock;
    TestSink sink;
    RemoteStateMachine machine(config, clock, sink);
    machine.setCameraReady(true); machine.setWifiReady(true); machine.start();
    std::string error;
