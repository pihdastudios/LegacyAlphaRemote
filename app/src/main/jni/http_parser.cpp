...
class JsonCursor {
  public:
    ...
    bool integerValue(int* out) {
        ...
        if (end == NULL || *end != '\0' || value < -2147483647L || value > 2147483647L)
            return false;
...
ParseResult parseHttpRequest(const std::string& data, const NativeConfig& config,
    ...
    if (lineEnd == std::string::npos)
        return data.size() > config.maxRequestLine ? PARSE_TOO_LARGE : PARSE_INCOMPLETE;
    if (lineEnd > config.maxRequestLine)
        return PARSE_TOO_LARGE;
    size_t firstSpace = data.find(' ');
    size_t secondSpace =
        firstSpace == std::string::npos ? std::string::npos : data.find(' ', firstSpace + 1);
    ...
    if ((request->method != "GET" && request->method != "POST") || !validPath(request->path))
        return PARSE_BAD_REQUEST;
    if (version != "HTTP/1.0" && version != "HTTP/1.1")
        return PARSE_UNSUPPORTED;
    for (size_t i = 0; i < request->method.size(); ++i)
        if (!isTokenChar(request->method[i]))
            return PARSE_BAD_REQUEST;

    size_t headerEnd = data.find("\r\n\r\n", lineEnd + 2);
    if (headerEnd == std::string::npos)
        return data.size() > config.maxHeaders ? PARSE_TOO_LARGE : PARSE_INCOMPLETE;
    if (headerEnd + 4 > config.maxHeaders)
        return PARSE_TOO_LARGE;
    request->pin.clear();
    ...
    while (at < headerEnd) {
        ...
        if (colon == std::string::npos || colon >= end || colon == at)
            return PARSE_BAD_REQUEST;
        ...
        if (lname == "content-length") {
            ...
            haveLength = true;
            if (contentLength > config.maxBody)
                return PARSE_TOO_LARGE;
        } else if (lname == "transfer-encoding") {
            if (!value.empty() && lower(value) != "identity")
                return PARSE_UNSUPPORTED;
        } else if (lname == "x-legacyalpha-pin") {
            if (!request->pin.empty() && request->pin != value)
                return PARSE_BAD_REQUEST;
            request->pin = value;
        } else if (lname == "content-type")
            request->contentType = lower(value);
        at = end + 2;
    }
...
bool parseCaptureJson(const std::string& body, CaptureRequest* request) {
    JsonCursor c(body);
    bool id = false, delay = false, autofocus = false;
    if (!c.take('{'))
        return false;
    if (c.take('}'))
        return false;
    for (;;) {
        std::string key;
        if (!c.stringValue(&key) || !c.take(':'))
            return false;
        if (key == "request_id") {
            if (id || !c.stringValue(&request->requestId))
                return false;
            id = true;
        } else if (key == "delay_ms") {
            if (delay || !c.integerValue(&request->delayMs))
                return false;
            delay = true;
        } else if (key == "autofocus") {
            if (autofocus || !c.boolValue(&request->autofocus))
                return false;
            autofocus = true;
        } else
            return false;
