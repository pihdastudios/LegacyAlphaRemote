...
class AndroidDispatcher : public JavaDispatcher {
  public:
    virtual bool dispatchToJava(const NativeCommand& command) {
        if (g_vm == NULL || g_activity == NULL || g_dispatch == NULL)
            return false;
        JNIEnv* env = NULL;
        bool attached = false;
        if (g_vm->GetEnv(reinterpret_cast<void**>(&env), JNI_VERSION_1_6) != JNI_OK) {
            ...
            if (g_vm->AttachCurrentThread(&env, NULL) != JNI_OK)
                return false;
#else
            if (g_vm->AttachCurrentThread(reinterpret_cast<void**>(&env), NULL) != JNI_OK)
                return false;
#endif
            attached = true;
        }
        ...
        env->CallVoidMethod(g_activity, g_dispatch, static_cast<jlong>(command.id),
                            static_cast<jint>(command.type), static_cast<jint>(command.argument),
                            text);
        if (text != NULL)
...
extern "C" JNIEXPORT jboolean JNICALL Java_io_pihda_legacyalpharemote_NativeBridge_nativeStart(
    JNIEnv* env, jclass, jobject activity, jstring logPath, jint port) {
    if (g_runtime != NULL)
        return JNI_TRUE;
...
Java_io_pihda_legacyalpharemote_NativeBridge_nativeSetCameraReady(JNIEnv*, jclass, jboolean ready) {
    if (g_runtime != NULL)
        g_runtime->setCameraReady(ready == JNI_TRUE);
}

extern "C" JNIEXPORT void JNICALL Java_io_pihda_legacyalpharemote_NativeBridge_nativeSetWifiState(
    JNIEnv* env, jclass, jboolean ready, jstring ssid, jstring password, jstring address,
...
Java_io_pihda_legacyalpharemote_NativeBridge_nativePhysicalFocus(JNIEnv*, jclass,
                                                                 jboolean pressed) {
    return g_runtime != NULL && g_runtime->physicalFocus(pressed == JNI_TRUE) ? JNI_TRUE
                                                                              : JNI_FALSE;
}

extern "C" JNIEXPORT jboolean JNICALL
Java_io_pihda_legacyalpharemote_NativeBridge_nativePhysicalShutter(JNIEnv*, jclass,
                                                                   jboolean pressed) {
    return g_runtime != NULL && g_runtime->physicalShutter(pressed == JNI_TRUE) ? JNI_TRUE
                                                                                : JNI_FALSE;
}

extern "C" JNIEXPORT void JNICALL Java_io_pihda_legacyalpharemote_NativeBridge_nativeSetPreview(
    JNIEnv* env, jclass, jbyteArray jpeg, jint width, jint height, jstring source, jstring error) {
    if (g_runtime == NULL)
        return;
    ...
    g_runtime->setPreview(reinterpret_cast<unsigned char*>(bytes), static_cast<size_t>(size), width,
                          height, s, e);
    if (bytes != NULL)
        env->ReleaseByteArrayElements(jpeg, bytes, JNI_ABORT);
    if (source != NULL)
