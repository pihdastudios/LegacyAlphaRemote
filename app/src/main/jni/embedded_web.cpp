...
"#photo{display:block;width:100%;height:auto;border-radius:8px;background:#090909}#photoMeta,#photoEmpty{color:#bbb;font-size:.8em}"
...
"conn=document.getElementById('connection'),result=document.getElementById('result'),photo=document.getElementById('photo'),"
"photoEmpty=document.getElementById('photoEmpty'),photoMeta=document.getElementById('photoMeta'),timer=null,backoff=5000,previewVersion=0,previewUrl='';"
...
"function loadPreview(s){var x=new XMLHttpRequest(),finished=false;x.open('GET','/api/v1/preview.jpg',true);x.timeout=8000;"
"x.responseType='blob';x.setRequestHeader('X-LegacyAlpha-Pin',pin.value);function finish(){if(finished||x.readyState!==4)return;finished=true;"
"if(x.status===200&&x.response){if(previewUrl)URL.revokeObjectURL(previewUrl);previewUrl=URL.createObjectURL(x.response);photo.src=previewUrl;"
"photo.hidden=false;photoEmpty.hidden=true;previewVersion=s.preview_version;photoMeta.textContent=(s.preview_source==='live_view_fallback'?'Live-view fallback':'Captured photo')+' · '+s.preview_width+'×'+s.preview_height;}"
"else{photoMeta.textContent=x.status===401?'Enter the session PIN to load the photo':'Preview unavailable';}}"
"x.onreadystatechange=finish;x.onerror=x.ontimeout=function(){if(!finished){finished=true;photoMeta.textContent='Preview unavailable';}};x.send(null);}"
...
"if(s.preview_available&&s.preview_version!==previewVersion)loadPreview(s);else if(s.preview_error)photoMeta.textContent=s.preview_error;"
