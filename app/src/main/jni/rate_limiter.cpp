...
RateLimiter::RateLimiter(double capacity, double refillPerSecond, size_t maximumKeys)
    : capacity_(capacity), refillPerMs_(refillPerSecond / 1000.0), maximumKeys_(maximumKeys) {}
...
bool RateLimiter::allow(const std::string& key, int64_t nowMs) {
    ...
    if (bucket == NULL) {
        if (buckets_.size() >= maximumKeys_)
            buckets_.erase(buckets_.begin());
        Bucket fresh;
        fresh.key = key;
        fresh.tokens = capacity_;
        fresh.lastMs = nowMs;
