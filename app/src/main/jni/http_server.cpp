...
HttpServer::HttpServer(const NativeConfig& config, HttpRequestHandler& handler)
    : config_(config), handler_(handler), threadCreated_(false), stopping_(false), running_(false),
      listener_(-1), nextListenAttemptMs_(0) {
...
bool HttpServer::start() {
    ...
    if (pipe(wakePipe_) != 0)
        return false;
    nonblocking(wakePipe_[0]);
    nonblocking(wakePipe_[1]);
    stopping_ = false;
...
void* HttpServer::threadEntry(void* value) {
    static_cast<HttpServer*>(value)->run();
    return NULL;
}
...
bool HttpServer::openListener(const std::string& address) {
    ...
    if (bind(fd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) != 0) {
        nativeLog("ERROR", std::string("bind failed on port ") + portText(config_.port) + ": " +
                               strerror(errno));
        close(fd);
        return false;
    }
    if (listen(fd, static_cast<int>(config_.maxClients)) != 0 || nonblocking(fd) != 0) {
        nativeLog("ERROR", std::string("listen setup failed on port ") + portText(config_.port) +
                               ": " + strerror(errno));
        close(fd);
        return false;
    }
...
void HttpServer::closeListener() {
    if (listener_ >= 0) {
        close(listener_);
        listener_ = -1;
        nativeLog("INFO", "server listener stopped");
    }
}
...
void HttpServer::acceptClient(int64_t nowMs) {
    ...
    if (fd < 0)
        return;
    if (clients_.size() >= config_.maxClients || nonblocking(fd) != 0) {
        close(fd);
        return;
    }
    ...
    client.remoteAddress =
        inet_ntop(AF_INET, &address.sin_addr, ip, sizeof(ip)) == NULL ? "unknown" : ip;
...
void HttpServer::readClient(size_t index, int64_t nowMs) {
    ...
    if (clients_[index].input.size() > config_.maxHeaders + config_.maxBody) {
        clients_[index].output = makeJsonError(413, "request too large");
        return;
    }
    HttpRequest request;
    size_t consumed = 0;
...
void HttpServer::writeClient(size_t index, int64_t nowMs) {
    ...
    if (client.written >= client.output.size()) {
        closeClient(index);
        return;
    }
...
#endif
    if (count > 0) {
        client.written += static_cast<size_t>(count);
        client.lastActivityMs = nowMs;
    } else if (count < 0 && errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
        closeClient(index);
        return;
    }
    if (index < clients_.size() && clients_[index].written == clients_[index].output.size())
        closeClient(index);
}
...
void HttpServer::run() {
    ...
    while (!stopping_) {
        ...
        for (size_t i = 0; i < clients_.size(); ++i) {
            struct pollfd p;
            p.fd = clients_[i].fd;
            p.events = clients_[i].output.empty() ? POLLIN : POLLOUT;
            p.revents = 0;
            descriptors.push_back(p);
        }
        int result = poll(&descriptors[0], descriptors.size(), 250);
        if (result < 0 && errno != EINTR)
            handler_.onServerError("HTTP poll failed");
        size_t offset = 0;
        if (descriptors[offset++].revents & POLLIN) {
            char drain[64];
        ...
        for (size_t remaining = clients_.size(); remaining > 0; --remaining) {
            ...
            if (events & (POLLERR | POLLHUP | POLLNVAL))
                ...
                readClient(i, clock.nowMs());
            else if (events & POLLOUT)
                writeClient(i, clock.nowMs());
        }
        const int64_t after = clock.nowMs();
        for (size_t i = 0; i < clients_.size();) {
            if (after - clients_[i].lastActivityMs > config_.idleTimeoutMs)
                closeClient(i);
            else
                ++i;
        }
    ...
    while (!clients_.empty())
        closeClient(0);
    running_ = false;
    nativeLog("INFO", "server thread stopped");
}
