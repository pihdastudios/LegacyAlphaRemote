...
public final class WifiDirectController {
    public interface Listener {
        void onWifiInfo(boolean ready, String ssid, String password,
    ...
    public WifiDirectController(Context context, Listener listener) {
        ...
        directManager = (DirectManager) context.getSystemService(
                DirectManager.WIFI_DIRECT_SERVICE);
        ...
        directStateReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context receiverContext, Intent intent) {
                int state = intent.getIntExtra(DirectManager.EXTRA_DIRECT_STATE,
                        DirectManager.DIRECT_STATE_UNKNOWN);
        ...
        groupSuccessReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context receiverContext, Intent intent) {
                DirectConfiguration configuration = (DirectConfiguration)
                        intent.getParcelableExtra(DirectManager.EXTRA_DIRECT_CONFIG);
    ...
    public void start() {
        ...
        registerReceivers();
        Logger.info("WiFiDirect: start wifiOriginallyEnabled="
                + wifiOriginallyEnabled + " directOriginallyEnabled="
                + directOriginallyEnabled);
        if (!wifiOriginallyEnabled && !wifiManager.setWifiEnabled(true)) {
            fail("Unable to enable Wi-Fi");
            return;
        }
        if (wifiOriginallyEnabled) enableDirect();
    }

    public void stop() {
        if (!started && !receiversRegistered) return;
        started = false;
    ...
    private void registerReceivers() {
        ...
        context.registerReceiver(directStateReceiver,
                new IntentFilter(DirectManager.DIRECT_STATE_CHANGED_ACTION));
        context.registerReceiver(groupSuccessReceiver,
                new IntentFilter(DirectManager.GROUP_CREATE_SUCCESS_ACTION));
        context.registerReceiver(groupFailureReceiver,
                new IntentFilter(DirectManager.GROUP_CREATE_FAILURE_ACTION));
        context.registerReceiver(stationConnectedReceiver,
                new IntentFilter(DirectManager.STA_CONNECTED_ACTION));
        context.registerReceiver(stationDisconnectedReceiver,
                new IntentFilter(DirectManager.STA_DISCONNECTED_ACTION));
    ...
    private void wifiStateChanged(int state) {
        ...
        if (started && state == WifiManager.WIFI_STATE_ENABLED) enableDirect();
    }
    ...
    private void directStateChanged(int state) {
        ...
        if (started && state == DirectManager.DIRECT_STATE_ENABLED) createGroup();
    }

    private void createGroup() {
        if (!started || groupStarted) return;
        try {
            ...
            DirectConfiguration configuration = (DirectConfiguration)
                    configurations.get(configurations.size() - 1);
    ...
    private void groupCreated(DirectConfiguration configuration) {
        ...
        Logger.info("WiFiDirect: group created ssid=" + ssid
                + " interface=" + valueOrEmpty(configuration.getIfName())
                + " address=" + address);
        publish(true, "");
    }
    ...
    private String normalizeAddress(String candidate) {
        if (candidate == null || candidate.length() == 0) return DEFAULT_ADDRESS;
