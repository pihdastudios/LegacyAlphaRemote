...
public final class LegacyAlphaRemoteActivity extends BaseActivity
        implements SonyCameraController.Listener, WifiDirectController.Listener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Logger.info("LegacyAlphaRemote: created version=0.1.0 native="
    ...
    protected void onResume() {
        ...
        Logger.info("LegacyAlphaRemote: onResume model=" + Build.MODEL
        ...
        nativeStarted = NativeBridge.nativeStart(this,
                Logger.getFile().getAbsolutePath(), HTTP_PORT);
        if (!nativeStarted) {
        ...
        mainHandler.post(new Runnable() {
            ...
            public void run() {
                if (commandType == NativeBridge.COMMAND_UPDATE_SCREEN) {
                    if (resumed) setStatus(stringArgument == null ? "" : stringArgument);
                    return;
                }
                if (!resumed) {
                    NativeBridge.nativeReportCommandResult(commandId, commandType,
                            -1, "activity paused");
                    return;
                }
    ...
    public void onCameraReadyChanged(boolean ready) {
        if (nativeStarted) NativeBridge.nativeSetCameraReady(ready);
    }

    @Override
    public void onCameraCommandResult(long commandId, int commandType,
    ...
    public void onCameraError(String message) {
        if (!nativeStarted) setStatus("Legacy Alpha Remote\n\n" + message);
    }
    ...
    public void onWifiInfo(boolean ready, String ssid, String password,
        ...
        if (nativeStarted) {
            NativeBridge.nativeSetWifiState(ready, ssid, password, address,
                    stationCount, error);
