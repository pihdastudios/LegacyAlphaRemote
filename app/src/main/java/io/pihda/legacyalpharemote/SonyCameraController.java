...
public final class SonyCameraController implements SurfaceHolder.Callback {
    ...
    public interface Listener {
        ...
        void onCameraPreview(byte[] jpeg, int width, int height,
    ...
    public void start() {
        ...
        previewWorker = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
    ...
    private void scheduleFallback(final int generation) {
        ...
        mainHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (!active || fallbackGeneration != generation) return;
                if (previewStarted) requestLiveViewFallback(generation);
            }
        }, JPEG_TIMEOUT_MS);
    }
    ...
    private void requestPendingFallback() {
        ...
        if (generation == 0 || generation != captureGeneration) return;
    ...
    private void requestLiveViewFallback(final int generation) {
        if (cameraEx == null || !previewStarted) return;
        try {
            cameraEx.getNormalCamera().setOneShotPreviewCallback(
                    new Camera.PreviewCallback() {
                @Override
                public void onPreviewFrame(byte[] data, Camera camera) {
                    if (fallbackGeneration != generation) return;
                    fallbackGeneration = 0;
                    fallbackDeadlineMs = 0L;
            ...
            fallbackGeneration = 0;
            fallbackDeadlineMs = 0L;
            publishPreviewError("live_view_fallback", "Fallback request failed: "
                    + e.toString());
        }
    }
    ...
    private void queueLiveView(final byte[] data, final int width, final int height,
            final int format) {
        submitPreviewTask(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (!image.compressToJpeg(new Rect(0, 0, width, height),
                            PREVIEW_JPEG_QUALITY, output)) {
    ...
    private void processJpeg(byte[] data, String source) {
        ...
        try {
            ...
            if (decoded == null) throw new IOException("JPEG decode failed");
            ...
            Logger.info("Camera: preview published source=" + source + " width="
                    + width + " height=" + height + " bytes=" + jpeg.length);
        } catch (Throwable e) {
